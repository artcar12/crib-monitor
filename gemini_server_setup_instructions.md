...
Follow README.md in the repo exactly: "Server install (Ubuntu)", then "On-server verification checklist". Where the README says `<this repo>`, use the URL above.
...
2. Do the "Server install (Ubuntu)" commands up to and including creating .env. Then stop for me to fill in .env (see HARD RULES).
